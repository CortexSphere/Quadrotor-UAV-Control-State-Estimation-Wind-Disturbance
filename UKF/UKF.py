...
class Estimator(object):
    def __init__(self, x_hat, u, y, P, Q, R, f_sys, h, Ts):
        ...
        self.h = lambda x: h(x)
        self.P_history = [self.P.copy()]  # 记录协方差矩阵的历史
    ...
    def sigmaPoints_SVD(self):
        ...
        X = np.zeros((2 * n + 1, n))
        """
        W_m = np.full(2 * n + 1, 1 / (2 * (n + 2)))
        W_c = np.full(2 * n + 1, 1 / (2 * (n + 2)))
        W_m[0] = 2 / (n + 2)
        W_c[0] = 2 / (n + 2) + (1 - 0.01 ** 2 + 2) 
        scaling = np.sqrt(n + 2)
        """
        W_m = np.full(2 * n + 1, 1 / (2 * (n + lamb)))
        W_c = np.full(2 * n + 1, 1 / (2 * (n + lamb)))
        W_m[0] = lamb / (n + lamb)
        W_c[0] = lamb / (n + lamb) + (1 - alpha ** 2 + beta)
        scaling = np.sqrt(n + lamb)
        # 确保协方差矩阵是对称的
...
def F(t, x):
    ...
    multiplicative_noise = noise_scale * y_true * np.random.normal(0, 0.03, size=y_true.shape)
    # 添加噪声
...
x_mse = np.sqrt(np.mean(np.square(xdot_est - xdot)))
y_mse = np.sqrt(np.mean(np.square(ydot_est - ydot)))
z_mse = np.sqrt(np.mean(np.square(zdot_est - zdot)))
